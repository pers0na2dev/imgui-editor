...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, ImColor(47, 47, 47), 4);
    window->DrawList->AddRect(check_bb.Min, check_bb.Max, ImColor(50, 50, 51), 4);

    if (*v)
    {
        const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
        RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad + 1, pad + 1), ImColor(0, 166, 255), square_sz - pad * 3.0f);
    }

    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x + 2, check_bb.Min.y + style.FramePadding.y), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(ImGui::GetWindowSize().x - 20, 30));
    ...
    window->DrawList->AddRectFilled(total_bb.Min, total_bb.Max, ImColor(47, 47, 47), 4);
    window->DrawList->AddRect(total_bb.Min, total_bb.Max, ImColor(50, 50, 51), 4);
    ...
    {
        window->DrawList->AddText(ImVec2(total_bb.Min.x + 10, total_bb.Min.y + 15 - label_size.y / 2 - 1), ImColor(255, 255, 255), preview_value);
    }
    if (label_size.x > 0)
        window->DrawList->AddText(ImVec2(total_bb.Max.x - label_size.x - 10, total_bb.Min.y + 15 - label_size.y/2 - 1), ImColor(220, 220, 220), label);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(100, 12), window->DC.CursorPos + ImVec2(ImGui::GetWindowSize().x - 60, 18));
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(ImGui::GetWindowSize().x - 20, 30));
    ...
    window->DrawList->AddRectFilled(total_bb.Min + ImVec2(90, 0), total_bb.Max, ImColor(47, 47, 47), 4);
    window->DrawList->AddRect(total_bb.Min + ImVec2(90, 0), total_bb.Max, ImColor(50, 50, 51), 4);

    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(39, 39, 39), 4);
    
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(frame_bb, id, data_type, p_data, p_min, p_max, format, flags, &grab_bb);

    if (grab_bb.Max.x > grab_bb.Min.x) {
        window->DrawList->AddRectFilled(frame_bb.Min, grab_bb.Min + ImVec2((grab_bb.Max.x - grab_bb.Min.x) / 2, 4), ImColor(45, 121, 255), 4);
        window->DrawList->AddCircleFilled(grab_bb.Min + ImVec2((grab_bb.Max.x - grab_bb.Min.x) / 2, 1), 4, ImColor(175, 175, 175), 32);
        window->DrawList->AddCircle(grab_bb.Min + ImVec2((grab_bb.Max.x - grab_bb.Min.x) / 2, 1), 4, ImColor(190, 190, 190, 90), 32);
    }

    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    window->DrawList->AddText(ImVec2(frame_bb.Max.x + 7, total_bb.Min.y + 6), ImColor(240, 240, 240), value_buf);

    if (label_size.x > 0.0f)
        window->DrawList->AddText(ImVec2(total_bb.Min.x, total_bb.Min.y + 15 - label_size.y/2), ImColor(240, 240, 240), label);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth() - 10, 20); // Arbitrary default of 8 lines high for multi-line
    const ImVec2 total_size = ImVec2(frame_size.x + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), frame_size.y);

    ImRect frame_bb;
    ImRect total_bb;

    if (!(flags & ImGuiInputTextFlags_Invisible)) {
        frame_bb = ImRect(window->DC.CursorPos + ImVec2(10, 5), window->DC.CursorPos + ImVec2(frame_size.x - label_size.x, 25));
        total_bb = ImRect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(frame_size.x, 34));
    }
    else {
        frame_bb = ImRect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(frame_size.x, 20));
        total_bb = ImRect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(frame_size.x, 20));
    }
    ...
    if (!(flags & ImGuiInputTextFlags_Invisible)) {
        if (label_size.x > 0)
            RenderText(ImVec2(total_bb.Max.x - label_size.x - 10, total_bb.Min.y + 15 - label_size.y/2), label);
    }
